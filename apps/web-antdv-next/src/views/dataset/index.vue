...
<template>
  <Page auto-content-height>
    <div class="flex h-full gap-4">
      ...
      <div class="w-56 shrink-0 overflow-auto rounded-lg bg-white p-3">
        <h3 class="mb-3 text-sm font-medium text-gray-600">数据分层</h3>
        <ul class="space-y-1">
          <li>
            <a
              class="flex cursor-pointer items-center gap-2 rounded-md px-3 py-2 text-sm transition-colors"
              :class="selectedLayerId === null ? 'bg-blue-50 text-blue-600' : 'text-gray-700 hover:bg-gray-50'"
          ...
          <li v-for="layer in layers" :key="layer.id">
            <a
              class="flex cursor-pointer items-center gap-2 rounded-md px-3 py-2 text-sm transition-colors"
              :class="selectedLayerId === layer.id ? 'bg-blue-50 text-blue-600' : 'text-gray-700 hover:bg-gray-50'"
